...
| **Hybrid** (Übergang) | Bewusst erlaubte Nähe zu Root-GUI-Brücken oder `app.gui.*`-Teilbäumen **bis** Ports/Contracts nachziehen; **kein** Freifahrtschein für neue Shell-Kopplung | `ui_application`, `global_overlay`, `workspace_presets`, `help`, `devtools` — Detail und Grenzen: [`SEGMENT_HYBRID_COUPLING_NOTES.md`](../architecture/SEGMENT_HYBRID_COUPLING_NOTES.md); Katalog im Code: `HYBRID_PRODUCT_SEGMENTS` in `tests/architecture/segment_dependency_rules.py` |
