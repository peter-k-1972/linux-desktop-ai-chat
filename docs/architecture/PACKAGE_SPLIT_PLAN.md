...
**Workflows-spezifischer Ist-Stand:** Quelle physisch [`linux-desktop-chat-workflows/src/app/workflows/`](../../linux-desktop-chat-workflows/src/app/workflows/) → Import `app.workflows` — Runbook [`PACKAGE_WORKFLOWS_PHYSICAL_SPLIT.md`](PACKAGE_WORKFLOWS_PHYSICAL_SPLIT.md). Weiterhin GUI-frei; dokumentierte Außenkanten liegen in `registry` / `execution.node_executors` zu `app.services.workflow_*_adapter`, `app.pipelines` / `app.pipelines.executors`, `app.prompts.prompt_service`, `app.agents.agent_service` sowie `app.chat.context_policies` / `app.chat.request_context_hints` (Adapter-/Vertragsrest, nicht Root-Public-Surface).
...
**Persistence-spezifischer Ist-Stand:** Quelle physisch [`linux-desktop-chat-persistence/src/app/persistence/`](../../linux-desktop-chat-persistence/src/app/persistence/) → Import `app.persistence` — Runbook [`PACKAGE_PERSISTENCE_PHYSICAL_SPLIT.md`](PACKAGE_PERSISTENCE_PHYSICAL_SPLIT.md). DB-Root-Auflösung wurde vor dem Cut von der alten Dateitiefe entkoppelt; Host `app/persistence/` ist entfernt, der Host bindet `linux-desktop-chat-persistence` per `file:`-Dependency, und die Split-Readiness-/SYSTEM_MAP-Nachzüge sind erfolgt.
...
| `workflows`, `projects`, `persistence` | `workflows`: `linux-desktop-chat-workflows/src/app/workflows/` → `app.workflows`; `projects`: `linux-desktop-chat-projects/src/app/projects/` → `app.projects`; `persistence`: `linux-desktop-chat-persistence/src/app/persistence/` → `app.persistence` | `ldc-workspace-data` | `core`, `utils` | Persistenz-/Workflow-Tests | Migration/ORM an Host-Editionen; **projects** — [`PACKAGE_PROJECTS_PHYSICAL_SPLIT.md`](PACKAGE_PROJECTS_PHYSICAL_SPLIT.md); **workflows** — [`PACKAGE_WORKFLOWS_PHYSICAL_SPLIT.md`](PACKAGE_WORKFLOWS_PHYSICAL_SPLIT.md); **persistence** — [`PACKAGE_PERSISTENCE_PHYSICAL_SPLIT.md`](PACKAGE_PERSISTENCE_PHYSICAL_SPLIT.md) |
