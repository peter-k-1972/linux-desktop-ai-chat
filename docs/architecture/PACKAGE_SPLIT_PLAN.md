...
| `workflows`, `projects`, `persistence` | `app/workflows/`; `projects`: `linux-desktop-chat-projects/src/app/projects/` → `app.projects`; `app/persistence/` | `ldc-workspace-data` | `core`, `utils` | Persistenz-/Workflow-Tests | Migration/ORM an Host-Editionen; **projects** physisch gesplittet — [`PACKAGE_PROJECTS_PHYSICAL_SPLIT.md`](PACKAGE_PROJECTS_PHYSICAL_SPLIT.md) |
