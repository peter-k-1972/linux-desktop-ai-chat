"""
QA Test Strategy Engine – Hilfsfunktionen.
...
# Projekt-Root: scripts/qa/test_strategy/ -> 4 Ebenen hoch
_TEST_STRATEGY_DIR = Path(__file__).resolve().parent
_QA_DIR = _TEST_STRATEGY_DIR.parent
...
def safe_get(data: dict[str, Any] | None, *keys: str, default: Any = None) -> Any:
    """Sichere verschachtelte Dict-Zugriffe."""
    if data is None:
        return default
    current: Any = data
    for key in keys:
