...
def _repo_root_for_matrix_checks() -> Path:
    """
    Verzeichnis, unter dem ``tests/unit/features/`` liegt (Host-Monorepo-Root).

    Bei installiertem Wheel liegt ``__file__`` unter ``site-packages`` — dort führt
    ein reines Hochlaufen nicht zum Monorepo. Reihenfolge: explizite Umgebungsvariable,
    ``cwd`` (typisch pytest/CI aus Repo-Root), dann Vorfahren von ``__file__``.
    """
    for key in ("LDC_REPO_ROOT", "GITHUB_WORKSPACE"):
        raw = (os.environ.get(key) or "").strip()
        if raw:
            p = Path(raw).resolve()
            if (p / "tests" / "unit" / "features").is_dir():
                return p
    try:
        cwd = Path.cwd().resolve()
        if (cwd / "tests" / "unit" / "features").is_dir():
            return cwd
    except OSError:
        pass
    here = Path(__file__).resolve()
    for ancestor in here.parents:
        if (ancestor / "tests" / "unit" / "features").is_dir():
            return ancestor
...
def validate_internal_plugin_smoke_consistency(
    *,
    ...
    ``repo_root``: Host-Monorepo-Wurzel (mit ``tests/unit/features/``). Wenn ``None``,
    :func:`_repo_root_for_matrix_checks` (Umgebung / cwd / Vorfahren von ``__file__``).

    Leere Liste = ok.
    """
    errors: list[str] = []
    er = edition_registry or build_default_edition_registry()
    root = repo_root if repo_root is not None else _repo_root_for_matrix_checks()
...
def validate_release_matrix_consistency(
    ...
    *,
    ...
    ``repo_root``: wird an :func:`validate_internal_plugin_smoke_consistency` durchgereicht
    (z. B. ``tools/ci/release_matrix_ci.py`` setzt die Host-Wurzel).
    ...
    errors.extend(
        validate_internal_plugin_smoke_consistency(edition_registry=er, repo_root=repo_root)
    )
