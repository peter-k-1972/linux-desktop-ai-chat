...
ohne ``chat_ops`` Legacy-Fallback wie zuvor.
"""
...
def build_chat_item_context_menu(
    ...
    *,
    ...
    rename_action.triggered.connect(
        lambda: _do_rename(chat_id, chat_title, parent_widget, on_action, chat_ops)
    )
    ...
    if project_id is not None:
        ...
        move_to_ungrouped.triggered.connect(
            lambda: _do_move_to_topic(project_id, chat_id, None, on_action, chat_ops)
        )
        if topics:
            ...
            for t in topics:
                ...
                action.triggered.connect(
                    lambda checked=False, tid=tid: _do_move_to_topic(
                        project_id, chat_id, tid, on_action, chat_ops
                    )
                )
        ...
        if is_pinned:
            ...
            pin_action.triggered.connect(
                lambda: _do_pin(project_id, chat_id, False, on_action, chat_ops)
            )
        else:
            pin_action = menu.addAction("Anheften")
            pin_action.setEnabled(not is_archived)
            pin_action.triggered.connect(
                lambda: _do_pin(project_id, chat_id, True, on_action, chat_ops)
            )

        if is_archived:
            archive_action = menu.addAction("Reaktivieren")
            archive_action.triggered.connect(
                lambda: _do_archive(project_id, chat_id, False, on_action, chat_ops)
            )
        else:
            archive_action = menu.addAction("Archivieren")
            archive_action.triggered.connect(
                lambda: _do_archive(project_id, chat_id, True, on_action, chat_ops)
            )
        ...
        dup_action.triggered.connect(
            lambda: _do_duplicate(project_id, chat_id, current_topic_id, on_action, chat_ops)
        )
    ...
    delete_action.triggered.connect(
        lambda cid=chat_id: _do_delete(
            cid, chat_title, parent_widget, on_action, on_chat_deleted, chat_ops
        )
    )
...
def _do_rename(
    ...
    try:
        if chat_ops is not None:
            chat_ops.save_chat_title(chat_id, title)
        else:
...
def _do_pin(
    ...
    try:
        if chat_ops is not None:
            chat_ops.set_chat_pinned(project_id, chat_id, pinned)
        else:
            from app.services.chat_service import get_chat_service
...
def _do_archive(
    ...
    try:
        if chat_ops is not None:
            chat_ops.set_chat_archived(project_id, chat_id, archived)
        else:
            from app.services.chat_service import get_chat_service
...
def _do_duplicate(
    ...
    try:
        if chat_ops is not None:
            new_id = chat_ops.duplicate_chat(chat_id, project_id, topic_id)
        else:
            from app.services.chat_service import get_chat_service
...
def build_context_bar_context_menu(
    ...
    *,
    ...
    if chat_id is not None:
        ...
        _populate_move_to_project_menu(
            move_project_sub, chat_id, project_id, on_action, chat_ops
        )
...
def _populate_move_to_project_menu(
    ...
    try:
        if chat_ops is not None:
            projects = chat_ops.list_projects_for_chat_move(current_project_id)
            has_any = bool(projects)
            for row in projects:
                pid = row.project_id
                action = submenu.addAction(row.name)
                action.triggered.connect(
                    lambda checked=False, pid=pid: _do_move_chat_to_project(
                        chat_id, pid, on_action, chat_ops
                    )
                )
        else:
...
def _do_move_chat_to_project(
    ...
    try:
        if chat_ops is not None:
            chat_ops.move_chat_to_project(chat_id, target_project_id)
            chat_ops.set_active_project_selection(target_project_id)
        else:
            from app.services.project_service import get_project_service
            ...
            get_project_context_manager().set_active_project(target_project_id)
        on_action()
    except Exception:
        pass
...
def _do_delete(
    ...
    parent_widget,
    on_action: Callable[[], None],
    on_chat_deleted: Optional[Callable[[int], None]],
    chat_ops: ChatOperationsPort | None,
) -> None:
