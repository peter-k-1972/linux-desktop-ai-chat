"""
AgentTasksWorkspace – Steuerungsraum für Agenten.

AgentRegistryPanel, AgentTaskPanel, ActiveAgentsPanel, AgentSummaryPanel.
Vollständig an AgentService, AgentTaskRunner und DebugStore angebunden.
"""
...
class AgentTasksWorkspace(BaseOperationsWorkspace):
    """Agent Tasks Arbeitsraum – Agenten sehen, starten, Aktivität nachvollziehen."""
    ...
    def _setup_ui(self):
        ...
        self._registry = AgentRegistryPanel(self)
        self._task_panel = AgentTaskPanel(self)
        self._active_panel = ActiveAgentsPanel(self)
        self._summary = AgentSummaryPanel(self)
        self._result_panel = AgentResultPanel(self)

        grid.addWidget(self._registry, 0, 0)
        grid.addWidget(self._task_panel, 0, 1)
        grid.addWidget(self._active_panel, 1, 0)
        grid.addWidget(self._summary, 1, 1)
        grid.addWidget(self._result_panel, 2, 0, 1, 2)
    ...
    def _on_project_context_changed(self, payload: dict) -> None:
        """Reload agent data and refresh UI when project changes."""
        self._refresh_agents()

    def _defer_init(self) -> None:
        """Lädt Agenten und stellt sicher, dass Seed-Agenten und DebugStore existieren."""
        try:
        ...
        try:
            ...
            get_debug_store()  # Sicherstellen, dass EventBus-Subscriber aktiv ist
        except Exception:
            pass
        self._refresh_agents()

    def _refresh_agents(self) -> None:
        """Aktualisiert Agentenliste in Registry und TaskPanel. Filtert nach aktivem Projekt."""
        try:
    ...
    def _on_agent_selected(self, profile) -> None:
        """Agent ausgewählt → Summary aktualisieren."""
    ...
    async def _run_task(self, agent_id: str, prompt: str) -> None:
        ...
        try:
            ...
            # ActiveAgentsPanel aktualisiert sich via Timer
    ...
    def setup_inspector(self, inspector_host, content_token: int | None = None) -> None:
        """Setzt Agent-Tasks-spezifischen Inspector."""
