...
def default_builtin_registry() -> ThemePackRegistry:
    """Builtin-Themes aus dem Paket ``app.ui_themes`` (Welle 7: eingebettete Distribution)."""
    import app.ui_themes

    pkg_dir = Path(app.ui_themes.__file__).resolve().parent
    reg = ThemePackRegistry()
    reg.register_root(pkg_dir / "builtins")
    return reg
